...
const getAllAuthors = async (req, res) => {
  try {
    ...
    res.status(200).json({ status: 1, message: "Successful", data: authors });
  } catch (error) {
    res.end("ok");
  }
};
...
const getAuthorByID = (req, res) => {
  ...
  authorsModel.findById(passedID).then((result)=>{
...
const addAuthor = (req, res) => {
  ...
  if (!email || !name) { // return if no id is present in params sent
    return res.status(400).json({ status: -1, message: `Email/Name not present`, data: [] });
  }
  authorsModel.findOne({email}).then((author)=>{ // check if the Author already exist
    if(author){
        return res.status(401).json({ status: -1, message: `Author already exist`, data: [] });
    }
    authorsModel.create({name, email}).then((data)=>{ // If Author does not exist, then add
        // remember to remove sensitive information
        res.status(200).json({ status: 1, message: `Author added sucessfully`, data: [data] });
    }).catch(err => {
        res.status(500).json({ status: -1, message: `${err}, try again!`, data: [] });
    })
    // let newAuthor = new authorsModel({name, email})
    // newAuthor.save().then(
    //     res.end('ok 4')
    // ).catch(()=>{
    //     res.end('ok 4')
...
const deleteAuthor = (req, res) => {
  ...
  authorsModel.findByIdAndDelete(passedID).then((result)=>{
    if(!result){ // throw error if no user found
        return res.status(400).json({ status: -1, message: `No author found!`, data: [] });
    }
    //remember to remove sensitive info like password here
...
const updateAuthor = async (req, res) => {
  ...
  try {
      const authorToUpdate = await authorsModel.findById(passedID)
      if(!authorToUpdate){
      ...
      if (fields.action == "password") {
        ...
        authorsModel.findByIdAndUpdate(passedID, newDetails).then(data => {
            if(!data){
                return res.status(404).json({ status: -1, message: `Author not found!`, data: [] });
            }
